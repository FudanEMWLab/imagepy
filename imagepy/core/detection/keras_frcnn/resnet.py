...
# 如果默认stride，w,h变为原来一半，深度为输入深度的残差块
    nb_filter1, nb_filter2, nb_filter3 = filters
    if K.image_dim_ordering() == 'tf':
        bn_axis = 3
    else:
...
# ResNet
    ...
    x = ZeroPadding2D((3, 3))(img_input)    # 上下左右补3排0，也就是长+6，宽+6

    x = Convolution2D(64, (7, 7), strides=(2, 2), name='conv1', trainable = trainable)(x)    # w,h /2
    #x = FixedBatchNormalization(axis=bn_axis, name='bn_conv1')(x)
    x = Activation('relu')(x)
    x = MaxPooling2D((3, 3), strides=(2, 2))(x)    # w,h /2

    # conv_block如果不输入stride，w，h大小边一半。identity_block w,h大小不变
    x = conv_block(x, 3, [64, 64, 256], stage=2, block='a', strides=(1, 1), trainable = trainable)    
    x = identity_block(x, 3, [64, 64, 256], stage=2, block='b', trainable = trainable)    
    x = identity_block(x, 3, [64, 64, 256], stage=2, block='c', trainable = trainable)    

    x = conv_block(x, 3, [128, 128, 512], stage=3, block='a', trainable = trainable)    # w,h /4
    x = identity_block(x, 3, [128, 128, 512], stage=3, block='b', trainable = trainable)
    x = identity_block(x, 3, [128, 128, 512], stage=3, block='c', trainable = trainable)
    x = identity_block(x, 3, [128, 128, 512], stage=3, block='d', trainable = trainable)

    x = conv_block(x, 3, [256, 256, 1024], stage=4, block='a', trainable = trainable)    # w,h /8
...
def classifier(base_layers, input_rois, num_rois, nb_classes = 21, trainable=False):
    ...
    # RoiPoolingConv输出了num_rois个7×7×channel的特征,classifier_layers是一个7x7的池化
